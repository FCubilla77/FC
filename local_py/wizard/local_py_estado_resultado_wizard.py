...
class LocalPyEstadoResultadoWizard(models.TransientModel):
    _name = 'local_py.estado_resultado.wizard'
    _description = 'Asistente Estado de Resultado'
    ...
    def action_ver_reporte(self):
        ...
        rows = builder._build_estado_resultado_rows(self.company_id, self.fecha, config)
        ...
        report_action = self.env.ref('local_py.action_report_estado_resultado_html')
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'Estado de Resultado (vista de control).html',
            'type': 'binary',
    ...
    def action_descargar_pdf(self):
        ...
        generacion = builder._generar_oficial(
            'estado_resultado', self.company_id, fecha_desde, self.fecha, config=config,
        )
        return {
            'type': 'ir.actions.act_url',
